...
class Parameter(metaclass=ParameterMeta):
    ...
    @classmethod
    def gen_and_save(cls, *args, **kwargs):
        """Generates parameters and saves them to the specified path.

        Args:
            *args: Additional arguments for parameter generation.
            **kwargs: Additional keyword arguments for parameter generation and saving. May include 'tag' for the file name.
        Examples:
            >>> Parameter.gen_and_save(10, saved_name='test_param')
        """
        file_path = f"{param_path}{cls.__name__}/"
        tag = kwargs.pop('tag', None)
        params = cls.gen(*args, **kwargs)

        saved_name = f"{cls.__name__}"
        if tag is not None:
            saved_name += f"_{tag}"
        for party_id in range(len(params)):
            name = 'p' + str(party_id) + '_' + saved_name + ".pth"
    ...
    def save(self, file_path, name):
        ...
        Args:
            ...
            name (str): The name of the file to save. Recommended to be the class name of the parameter.
