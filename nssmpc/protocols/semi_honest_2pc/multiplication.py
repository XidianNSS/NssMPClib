...
class AssMulTriples(Parameter):
    ...
    def __init__(self,
                 a: AdditiveSecretSharing = None,
                 b: AdditiveSecretSharing = None,
                 c: AdditiveSecretSharing = None,
                 size: int = 0):
        self.a = a
        self.b = b
        self.c = c
...
class MatmulTriples(Parameter):
    ...
    def __init__(self, a=None, b=None, c=None, size=0):
        ...
        self.c = c
        self.size = size
        # super().__init__(tag=f'{list(a.shape)}_{list(b.shape)}')
    ...
    def gen_and_save(cls, num_of_triples, x_shape=None, y_shape=None, num_of_party=2):
        ...
        """
        x_shape, y_shape = align_shape(x_shape, y_shape)
        triples = cls.gen(num_of_triples, x_shape, y_shape, num_of_party)
        for party_id in range(num_of_party):
            file_path = param_path + "MatmulTriples/"
            file_name = f"p{party_id}_MatmulTriples_{list(x_shape)}_{list(y_shape)}.pth"
            triples[party_id].save(file_path, file_name)
...
def gen_triples_by_ttp(num_of_triples: int, num_of_party: int, share_type: str = 'normal'):
    ...
    for i in range(num_of_party):
        triples.append(
            AssMulTriples(a=a_list[i].to('cpu'), b=b_list[i].to('cpu'), c=c_list[i].to('cpu'), size=num_of_triples))
    return triples
...
def gen_triples_by_homomorphic_encryption(num_of_triples: int, party) -> AssMulTriples:
    ...
    triples = AssMulTriples(a=AdditiveSecretSharing(RingTensor(a).to('cpu')),
                            b=AdditiveSecretSharing(RingTensor(b).to('cpu')),
                            c=AdditiveSecretSharing(RingTensor(c).to('cpu')),
                            size=num_of_triples)
...
def gen_matrix_triples_by_ttp(num_of_param: int, x_shape: Tuple, y_shape: Tuple, num_of_party: int = 2):
    ...
    triples = []
    for i in range(num_of_party):
        triples.append(MatmulTriples(a_list[i].to('cpu'), b_list[i].to('cpu'), c_list[i].to('cpu')))

    return triples


def sh2pc_mul_beaver(x: AdditiveSecretSharing, y: AdditiveSecretSharing, party: Party = None) -> AdditiveSecretSharing:
    """
    ...
    Examples:
        >>> res = sh2pc_mul_beaver(x, y)
    """
    assert x.shape == y.shape, "Shapes of x and y must be equal"  # TODO: broadcasting
    if party is None:
        party = PartyCtx.get()
    shape = x.shape
    x = x.flatten()
    y = y.flatten()
    a, b, c = party.get_param(AssMulTriples, x.numel())
    a.dtype = b.dtype = c.dtype = x.dtype
    e = x - a
    f = y - b

    e_and_f = AdditiveSecretSharing.cat([e, f], dim=0)
    common_e_f = e_and_f.recon()
    common_e = common_e_f[:x.numel()]
    common_f = common_e_f[x.numel():]
    ...
    return AdditiveSecretSharing(res).reshape(shape)


def sh2pc_matmul_beaver(x: AdditiveSecretSharing, y: AdditiveSecretSharing,
    ...
    a_matrix, b_matrix, c_matrix = party.get_param(MatmulTriples, 1, tag=f'{x_shape}_{y_shape}')

    e = x - a_matrix
    f = y - b_matrix

    e_and_f = AdditiveSecretSharing.cat([e.flatten(), f.flatten()], dim=0)
    common_e_f = e_and_f.recon()
    ...
    return AdditiveSecretSharing(res)
