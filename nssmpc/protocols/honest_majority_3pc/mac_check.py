...
from nssmpc.protocols.honest_majority_3pc.base import hm3pc_open, hm3pc_coin
from nssmpc.protocols.honest_majority_3pc.base import rand_like
from nssmpc.protocols.semi_honest_3pc.multiplication import sh3pc_mul_with_out_trunc
...
def hm3pc_mac_check(x: ReplicatedSecretSharing, mx: ReplicatedSecretSharing, mac_key: ReplicatedSecretSharing,
    ...
    Examples:
        >>> hm3pc_mac_check(x_share, mac_share, key_share)
    """
    if party is None:
        party = PartyCtx.get()
    r = rand_like(x, party)
    mr = sh3pc_mul_with_out_trunc(r, mac_key)
    ro = hm3pc_coin(x.numel(), party).reshape(x.shape)
    v = r + x * ro
    w = mr + mx * ro
    v = hm3pc_open(v)
    hm3pc_check_zero(w - mac_key * v)


def hm3pc_check_zero(x: ReplicatedSecretSharing, party: Party | None = None) -> int:
    """
    ...
    Examples:
        >>> is_zero = hm3pc_check_zero(x_share)
    """
    if party is None:
        party = PartyCtx.get()
    # print("x restore", x.restore())
    r = rand_like(x, party)
    w = sh3pc_mul_with_out_trunc(x, r)
    w_open = hm3pc_open(w)
