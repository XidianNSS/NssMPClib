...
def open(x: ReplicatedSecretSharing, party: Party | None = None) -> RingTensor:
    """
...
def coin(num_of_value: int | list, party) -> ReplicatedSecretSharing:
    """
...
def recon(x: ReplicatedSecretSharing, target_party_id: int, party: Party = None) -> Optional[ReplicatedSecretSharing]:
    """
    ...
    Args:
        ...
        target_party_id: The ID of the party that will receive the reconstructed secret.
        party: The party instance. If None, uses the current party context.
    ...
    Examples:
        >>> x_val = recon(x_share, target_party_id=0)
    """
    if party is None:
        party = PartyCtx.get()
    # P_{i+1} send x_{i+2} to P_{i}
    if party.party_id == (target_party_id + 1) % 3:
        party.send(target_party_id, x.item[1])
    # P_{i-1} send x_{i+2} to P_{i}
    elif party.party_id == (target_party_id + 2) % 3:
        party.send(target_party_id, x.item[0])

    elif party.party_id == target_party_id:
        x_2_0 = party.recv((party.party_id + 2) % 3)
        x_2_1 = party.recv((party.party_id + 1) % 3)
        ...
        if cmp != 0:
            # raise ValueError("The two parties' calculations do not agree, and there may be a malicious party involved!")
            pass
        if party.party_id == target_party_id:
            return x.item[0] + x.item[1] + x_2_0
    return None


def receive_share_from(input_id: int, party: Party) -> ReplicatedSecretSharing:
    """
    ...
    Examples:
        >>> x_share = receive_share_from(input_id=0, party=party)
    """
...
def share(x: RingTensor, party) -> ReplicatedSecretSharing:
    """
