...
class VOSKey(Parameter):
    ...
    def load_provider(cls, party):
        party.virtual_party_with_previous.append_provider(
            ParamProvider(cls, saved_name=f'VOSKey_{(party.party_id + 1) % 3}_0'))
        party.virtual_party_with_next.append_provider(
            ParamProvider(cls, saved_name=f'VOSKey_{(party.party_id + 2) % 3}_1'))
        party.append_provider(
            ParamProvider(cls, saved_name=f'VOSKey_{party.party_id}_0', param_tag=f'VOSKey_{party.party_id}_0'))
        party.append_provider(
            ParamProvider(cls, saved_name=f'VOSKey_{party.party_id}_1', param_tag=f'VOSKey_{party.party_id}_1'))
