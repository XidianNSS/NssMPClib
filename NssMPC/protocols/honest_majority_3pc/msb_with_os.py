...
def msb_with_os_without_mac_check(x: ReplicatedSecretSharing, share_table=None, party: Party3PC = None):
    ...
    Examples:
        >>> v, mac_v, mac_key = msb_with_os_without_mac_check(x)
    """
    if party is None:
        party = PartyCtx.get()
    shape = x.shape
    ...
    self_rdx0, _, _ = party.get_param(VSigmaKey, num, tag=f'VSigmaKey_{party.party_id}_0')
    self_rdx1, _, _ = party.get_param(VSigmaKey, num, tag=f'VSigmaKey_{party.party_id}_1')
