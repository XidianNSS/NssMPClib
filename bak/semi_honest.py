...
#         Then :meth:`~NssMPC.infra.communication.communicator.Communicator.set_max_connections` is used to set the maximum
#         number of connections to 1, :meth:`set_communicator_address` is used to set the target communication address and
#         port, :meth:`~NssMPC.infra.communication.communicator.NCommunicator.init_server()` is used to initialize the server, and :meth:`~NssMPC.infra.communication.communicator.NCommunicator.connect_to_other` starts connecting to other entities.
#         Then call :meth:`check_all_parties_online` to verify that all participants are online. The auxiliary parameters are loaded by calling :meth:`~NssMPC.runtime.party.party.Party.load_aux_params`.
...
#
#         Use the :meth:`~NssMPC.infra.communication.communicator.Communicator.send_to_address` method of ``self.communicator`` to send data to the address specified by ``self_target_address``.
#
...
#
#         Call the :meth:`~NssMPC.infra.communication.communicator.Communicator.recv_from_address` method of ``self.communicator`` to receive data from ``self.receiving_address``.
...
#
#         First, the :meth:`~NssMPC.infra.communication.communicator.Communicator.close` method is called to close the TCP connection with the other
